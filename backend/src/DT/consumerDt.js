...
const kafka = new Kafka({
    clientId: "dt-consumer",
    brokers: ["kafka:9092"],
});

// This consumer delivers updates to websocket clients (fast path)
const consumer = kafka.consumer({ groupId: "dt-ws" });

export const runDtConsumer = async (io) => {
    await consumer.connect();
    await consumer.subscribe({ topic: "Dt", fromBeginning: true });
    console.log("Consumer Dt suscrito al topic 'Dt'");
    ...
    const flushBuffers = () => {
        ...
        for (const [key, buf] of buffers.entries()) {
            ...
            // Usamos event 'dt_update' para consistencia con 'opal_update'
            io.emit('dt_update', averaged);
        }
    ...
    await consumer.run({
        eachMessage: async ({ message }) => {
            try {
                ...
                console.error('Error processing Dt message in consumer:', err);
            }
        },
    });
};
